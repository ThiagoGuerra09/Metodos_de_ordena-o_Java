    ...
    public void Heapsortt() {
     ...
     public void Quick_sort() {
      ...
      private void quicksort(int esq, int dir) {
          int i = esq, j = dir;
          Series_Filmes pivo = series[(dir+esq)/2];
          while (i <= j) {
              while (series[i].getPais().compareTo(pivo.getPais()) < 0 || (series[i].getPais().compareTo(pivo.getPais()) == 0 && series[i].getNome().compareTo(pivo.getNome()) < 0 )) i++;     //|| (series[i].getPais().compareTo(pivo.getPais()) == 0 && series[i].getNome().compareTo(pivo.getNome()) < 0 )
              while (series[j].getPais().compareTo(pivo.getPais()) > 0 || (series[j].getPais().compareTo(pivo.getPais()) == 0 && series[j].getNome().compareTo(pivo.getNome()) > 0 )) j--;    // || (series[i].getPais().compareTo(pivo.getPais()) == 0 && series[i].getNome().compareTo(pivo.getNome()) > 0 )
            //   while (series[i].getPais().compareTo(pivo.getPais()) == 0 && series[i].getNome().compareTo(pivo.getNome()) < 0)i++;
            //   while (series[i].getPais().compareTo(pivo.getPais()) == 0 && series[i].getNome().compareTo(pivo.getNome()) > 0)j++;
              if (i <= j) {
                  swap(i, j);
                  i++;
                  j--;
      ...
      public void Bolha() {
		for (int i = (n - 1); i > 0; i--) {
			for (int j = 0; j < i; j++) {
				if (series[j].getTemporadas() > series[j + 1].getTemporadas() || (series[j].getTemporadas() == series[j + 1].getTemporadas() && series[j].getNome().compareTo(series[j + 1].getNome()) > 0 )) {
               swap(j, j+1);
				}
			}
		}
