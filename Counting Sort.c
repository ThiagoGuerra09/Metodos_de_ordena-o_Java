...
void selecao() {
    ...
    for (i=0;i < (n - 1); i++) {
      ...
      for (j= (i+1);j < n; j++){
        ...
        //     menor = j;
        //  }
          if (series[menor].num_temporadas==series[j].num_temporadas && strcmp (series[menor].nome,series[j].nome)>0){
         menor = j;
         }
...
void countingsort() {
    ...
    int j = 0;
    for ( j = 0; j < n; count[series[j].num_temporadas]++, j++);

    //Agora, o count[i] contem o numero de elemento menores ou iguais a i
    int k = 1;
    for( k = 1; k < tamCount; count[k] += count[k-1], k++);

    //Ordenando
    int h = n-1;
    for( h = n-1; h >= 0; ordenado[count[series[h].num_temporadas]-1] = series[h], count[series[h].num_temporadas]--, h--);
    
    //Copiando para o series original
    int l = 0;
    for( l = 0; l < n; series[l] = ordenado[l], l++);
